...
export default function MasterKeyModal({ onConfirm }: MasterKeyModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xl">
      ...
      >
        <div className="w-20 h-20 bg-[var(--accent)] rounded-[24px] shadow-xl shadow-blue-500/20 flex items-center justify-center -mt-20 text-[var(--accent-foreground)] transform rotate-6 hover:rotate-0 transition-transform duration-500">
        ...
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-3xl p-5 flex gap-4 text-left">
          <div className="w-10 h-10 bg-blue-500/20 rounded-2xl flex items-center justify-center shrink-0">
            <ShieldAlert className="w-5 h-5 text-blue-500" />
          </div>
          <p className="text-[11px] text-blue-500 dark:text-blue-400 font-semibold leading-relaxed">
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-6">
          ...
          <button
            type="submit"
            className="w-full bg-[var(--accent)] text-[var(--accent-foreground)] py-5 rounded-2xl flex items-center justify-center gap-3 font-bold text-sm shadow-xl shadow-blue-500/10 group transition-all hover:brightness-110 active:scale-95"
          >
